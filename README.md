...
현재 버전은 기본 샘플 데이터와 함께 **NAVER API HUB 공식 지역 검색**으로 사용자가 입력한 전국 지역의 숙소 후보를 가져옵니다. 에어비앤비는 파트너 승인 전까지 입력 지역·날짜·인원의 공식 검색 링크만 제공합니다. 실데이터 사진은 저작권과 API 범위를 지키기 위해 `사진 미제공` 상태로 표시합니다.
...
- NAVER API HUB 서버 측 인증, 지역별 6개·최대 30건, 지역·업종 검증, 중복 제거, 10분 single-flight 캐시, 글로벌 fetch 3개·IP별 검색 10회/분 제한
- 에어비앤비 입력 지역·체크인·체크아웃·인원 공식 검색 외부 링크와 파트너 승인 대기 상태 표시
- 커플/가족/친구 목적별 설명 가능한 추천
- 전국 지역과 날짜 검색, 가격, 욕실, 인원, 최소 평점, 6개 편의시설 복합 필터
- 추천/가격/거리 정렬
...
│   ├── search.js           # 지역·날짜·인원 검색 조건 검증
│   ├── presentation.js     # 가격과 제한 값 표시 포맷
...
- 샘플/평점 제공 데이터에는 기본 4.5 필터를 적용합니다. NAVER는 별점 필드가 없어 높은 별점 추천으로 표시하지 않고 `평점 미검증 후보`로 분리합니다.
